...
- [x] T002 Update `src/framework/state/BUILD.bazel` with `deps = ["//src/framework/core"]` and same visibility
- [x] T003 Create `src/framework/widgets/BUILD.bazel` as `cc_library` with deps on `//src/framework/core`, `//src/framework/state`, `@yoga//:yoga`
- [x] T004 Update `src/framework/public/BUILD.bazel` to add `//src/framework/core`, `//src/framework/state` to deps

**Checkpoint**: `bazel build //src/framework/core //src/framework/state //src/framework/widgets //src/framework/public:native_ui` succeeds
...
- [x] T013 [P] [US2] Create `src/framework/state/property_base.h` with `PropertyBase` abstract class — virtual `Signal()`, `key()` identity method
- [x] T014 [P] [US2] Create `src/framework/state/property.h` with `Property<T> : PropertyBase` template — constructor taking `State*`, `operator=` (triggers `before_set_` → value update → `Signal()` → `after_set_`), `value()`, `operator const T&()`, private `value_`, `before_set_` hook, `after_set_` hook
...
- [x] T016 [US2] Create `src/framework/state/state.h` with `State` class — `AddWatcher(key, callback, PropertyBase*)`, `RemoveWatcher(key)`, `EnqueueDirty`, `Flush`, mutex-protected watchers and dirty queue
- [x] T017 [US2] Create `src/framework/state/state.cc` with `State` implementation — `AddWatcher`/`RemoveWatcher`, `Flush` deduplicates and calls `NotifyWatchers`, each watcher invokes stored callback
...
- [x] T031 Create `src/framework/public/include/native_ui/state.h` — re-export `State`, `Property<T>`
